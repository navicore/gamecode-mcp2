...
impl ConversationManager {
    ...
    pub fn add_assistant_message_with_tools(
        ...
        self.add_message(Role::Assistant, content, Some(tool_calls), Some(tool_results));
    }
    ...
    fn trim_to_fit(&mut self) {
        while self.current_tokens > self.max_context_tokens && self.messages.len() > 1 {
            ...
            if let Some(pos) = self.messages.iter().position(|m| !matches!(m.role, Role::System)) {
    ...
    pub fn get_tool_history(&self) -> Vec<(Vec<crate::prompts::ToolCall>, Vec<ToolResult>)> {
        self.messages
            .iter()
            .filter_map(|msg| {
                match (&msg.tool_calls, &msg.tool_results) {
